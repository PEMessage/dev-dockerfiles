...
RUN sed -i "s@http://dl-cdn.alpinelinux.org/@https://repo.huaweicloud.com/@g" /etc/apk/repositories && apk update
...
# System tools
RUN apk add build-base cmake bison automake autoconf tcl\
        ncurses-dev ncurses-static \
        ca-certificates \
        wget curl \
        sed gawk ed bc perl grep\
        util-linux libtool coreutils file flex\
        shadow lsb-release\
        zlib tar gzip bzip2 xz

# Dev tools
RUN apk add man-db man-db-doc groff gdbm\
        linux-headers \
        musl-dev musl-utils \
        qemu-system-x86_64 \
        qemu-system-aarch64 \
...
RUN apk add sudo shadow libuser &&\
    addgroup "${USER}-group" &&\
    adduser -D -s /bin/zsh $USER && echo "$USER:$PASS" | chpasswd &&\
